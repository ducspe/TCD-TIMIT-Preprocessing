...
def calculate_optical_flow(video_file):
    of_cap = cv2.VideoCapture(video_file)
    dda_fullface_file = skvideo.io.vread(video_file)
    optical_flow_array = np.zeros((dda_fullface_file.shape[0], 67, 67))
    ...
    while ret:  # while frame was successfully returned
        ...
        if SHOW_OPTICAL_FLOW:
            ...
            if k == 27:  # if escape is pressed
                ...
                cv2.imwrite('grayfullface.png', of_frame2)
...
def my_processing_per_cpu_process(speaker_sublist):
    ...
    for speaker in speaker_sublist:
        ...
        for video_mp4_file, audio_wav_file in zip(speaker_mp4_files, speaker_audio_files):
            ...
            original_video_length = int(original_video_array.get(cv2.CAP_PROP_FRAME_COUNT))

            success1, frame1 = original_video_array.read()
            count_frame1 = 0

            full_faces = np.zeros((original_video_length, 67, 67), dtype=np.uint8)
            while success1:
                frame1 = cv2.resize(frame1, (67, 67), interpolation=cv2.INTER_AREA)
                # INTER_AREA is best for downsampling
                frame1 = cv2.cvtColor(frame1, cv2.COLOR_BGR2GRAY)

                full_faces[count_frame1] = frame1

                success1, frame1 = original_video_array.read()
                count_frame1 += 1

            ############################################################################################################
            # Create the dda video with gray full faces:

            os.makedirs(f'fullface_preprocessing_steps/{speaker}/', exist_ok=True)  # speaker is set by main, not in this function
            dda_gray_fullface_video = f'fullface_preprocessing_steps/{speaker}/{video_mp4_pa.split("/")[-1].replace(".mp4", "_dda_gray_fullface.mp4")}'
            video_writer0 = skvideo.io.FFmpegWriter(dda_gray_fullface_video,
            ...
            for fullface_frame in full_faces:
                video_writer0.writeFrame(fullface_frame)

            video_writer0.close()

            ############################################################################################################
            # Calculate the optical flow for the dda full face video and store it in npy file:
            dda_optical_flow = calculate_optical_flow(dda_gray_fullface_video)
            os.makedirs(f'dda_optical_flow_fullface_dataout/{speaker}/', exist_ok=True)
            dda_optical_flow_out_path = f'dda_optical_flow_fullface_dataout/{speaker}/{dda_gray_fullface_video.split("/")[-1].replace("_dda_gray_fullface.mp4", ".npy")}'
            np.save(dda_optical_flow_out_path, dda_optical_flow)  # this is saved as float64

            ############################################################################################################
            # Store the gray full face frames from the mp4 file to npy files:
            dda_fullface_file_reread = skvideo.io.vread(dda_gray_fullface_video)
            os.makedirs(f'dda_gray_fullface_dataout/{speaker}/', exist_ok=True)
            dda_gray_fullface_path = f'dda_gray_fullface_dataout/{speaker}/{dda_gray_fullface_video.split("/")[-1].replace("_dda_gray_fullface.mp4", ".npy")}'
            ...
            white_dda_array = skvideo.io.vread(dda_gray_fullface_video)
            ...
            video = ffmpeg.input(dda_gray_fullface_video.replace(".mp4", "_white.mp4"))
            audio = ffmpeg.input(audio_wav_pa)
            out = ffmpeg.output(video, audio, dda_gray_fullface_video.replace(".mp4", "_white_withaudio.mp4"), vcodec='copy',
...
if __name__ == "__main__":
    ...
    print(f"Elapsed time for the full face dataset creation: {(end_time - start_time) / 3600} hours")
