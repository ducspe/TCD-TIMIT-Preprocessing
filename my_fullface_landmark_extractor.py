...
def my_processing_per_cpu_process(speaker_sublist):
    ...
    for speaker in speaker_sublist:
        ...
        for video_mp4_file, audio_wav_file in zip(speaker_mp4_files, speaker_audio_files):
            ...
            facial_landmarks_arr = np.zeros((dda_video_length, end_roi_index-start_roi_index, 2), dtype=int)  # shape is (nr_frames, nr_landmarks_per_frame, 2 coordinates of a landmark)
            while success:
                ...
                for (count_face, detected_face) in enumerate(detected_faces):
                    ...
                    facial_landmarks_arr[count_frame] = np.nan_to_num(landmarks_arr[start_roi_index:end_roi_index])
            ...
            print("Facial landmarks array before saving: ", facial_landmarks_arr)
            np.save(dda_landmarks_path, facial_landmarks_arr)
